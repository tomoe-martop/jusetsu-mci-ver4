...
1 実行に複数タスクがある場合、N1/N2 に添えるログパスは実行末尾に 1 回だけ退避する共有ログ（全タスク分を含む）です。特定 task_id と誤認しないよう、複数タスク時の退避ファイル名は `predictor_unknown_...` になります。最外殻の例外で N3 を送る際に保留中の N1/N2 があれば、ログパス無しで先に送ります。
