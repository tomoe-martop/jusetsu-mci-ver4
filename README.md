...
| 事象 | 扱い |
|---|---|
| 接続失敗・接続／読み取りタイムアウト・本文受信中の切断（`ChunkedEncodingError`／`ContentDecodingError`）・HTTP 5xx・HTTP 429 | 再送（最大 5 回送信、固定 2 秒待ち）。枯渇したら `EgpfRetryExhausted` |
...
- **打ち切り**: 連続 3 リクエストでリトライが枯渇した場合は EGPF 障害の疑いとしてタスクを打ち切ります。処理中のハウスは `-1` 確定、残りのハウスは未処理のまま（`task_houses` は更新しない）、`tasks.status=-1` を書いて通知します。復旧後はダッシュボードから再実行タスクを作成してください。最終ハウスで閾値に達した場合（未処理ハウス 0 件）は打ち切らず、通常の完了処理（`tasks.status=1`、失敗があれば N1 通知）になります
...
| # | イベント | タイミング |
|---|---|---|
| N1 | タスク完了時に失敗ハウスあり（失敗 0 件なら通知しない） | ログの GCS アップロード後 |
| N2 | EGPF 全断によるタスク打ち切り | 同上 |
| N3 | タスク単位／最外殻の例外（DB 接続失敗等） | 発生時 |

通知はお客様も読むため、GCS のログパスなど技術的な表記は本文に載せません。ログは `gs://<GCS_LOG_BUCKET>/logs/predictor_<task_id 10桁>_<日時>.log` に退避されるので、通知の `task_id` から探せます（例: `gcloud storage ls gs://prd-mci-ver4/logs/ | grep 0000000433`）。1 実行に複数タスクがある場合は実行末尾に 1 回だけ退避する共有ログ（全タスク分を含む）になり、ファイル名は `predictor_unknown_...` です。最外殻の例外で N3 を送る際に保留中の N1/N2 があれば先に送ります。
